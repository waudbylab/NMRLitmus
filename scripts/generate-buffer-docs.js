...
for (const buffer of db.buffers) {
  ...
  const bufKey = `${buffer.sample_id}__${buffer.buffer_name}`;
  html += `    <div class="buffer-entry" id="${bid}" data-buffer-key="${bufKey}" data-buffer-name="${buffer.buffer_name}">
`;
...
`;
  ...
  // Plots injected at runtime by the client-side script below
  html += `      <div class="buffer-plots"></div>
`;
...
html += `
  ...
  (function () {
    ...
    document.getElementById('docs-lb-prev').addEventListener('click', e => {
      e.stopPropagation();
      lbIdx = (lbIdx - 1 + currentThumbs.length) % currentThumbs.length; openLb();
    });
    document.getElementById('docs-lb-next').addEventListener('click', e => {
      e.stopPropagation();
      lbIdx = (lbIdx + 1) % currentThumbs.length; openLb();
    });
    ...
    // ── Load plots from manifest at runtime ───────────────────────────────────
    (async function () {
      let manifest;
      try {
        const res = await fetch('../plots/manifest.json');
        if (!res.ok) return;
        manifest = await res.json();
      } catch (_) { return; }

      // Build lookup: sample_id__buffer_name → { dir, resonances[] }
      const plotsMap = new Map();
      for (const sample of manifest.samples || []) {
        for (const buf of sample.buffers || []) {
          plotsMap.set(sample.sample_id + '__' + buf.buffer_name, buf);
        }
      }

      // Inject plots into each buffer entry
      document.querySelectorAll('[data-buffer-key]').forEach(entry => {
        const plotData = plotsMap.get(entry.dataset.bufferKey);
